...
import { Heart, MessageSquare, Clock, Share2, MoreVertical, Trash2, Pencil, Flag } from 'lucide-react';
...
export default function CommunityCard({ post, currentUserId, onLike, onDelete, onEdit, onTagClick }: Props) {
  ...
  return (
    <>
        ...
        <div className="flex items-center justify-between">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="glass border-border/50">
              {isOwner && onEdit && <DropdownMenuItem onClick={() => onEdit(post)}><Pencil className="w-3 h-3 mr-2" />Edit</DropdownMenuItem>}
              {isOwner && onDelete && <DropdownMenuItem onClick={() => onDelete(post.id)} className="text-destructive"><Trash2 className="w-3 h-3 mr-2" />Delete</DropdownMenuItem>}
              {!isOwner && currentUserId && (
                <DropdownMenuItem onClick={() => setShowReport(true)}><Flag className="w-3 h-3 mr-2" />Report</DropdownMenuItem>
              )}
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
      ...
      <ReportModal open={showReport} onOpenChange={setShowReport} targetType="post" targetId={post.id} />
    </>
  );
}
